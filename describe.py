...
def calculate_statistics(column):
    column = utils.array(column, dtype=float)
    count = len(column)
    mean = utils.mean(column)
    std = utils.std(column)
    min_val = utils.min(column)
    q25 = utils.percentile(column, 25)
    median = utils.median(column)
    q75 = utils.percentile(column, 75)
    max_val = utils.max(column)
